...
--Q1. List all the states in which we have customers who have bought cellphones fom 2005 till today?
...
select
State
from FACT_TRANSACTIONS as t inner join DIM_LOCATION as l on t.IDLocation = l.IDLocation
where YEAR(Date) >= 2005
group by State

--Q2. Which state in the US is buying more 'Samsung' cell phones?

select top 1 State,
count(State) as Frequency
from FACT_TRANSACTIONS as t inner join DIM_LOCATION as l on t.IDLocation = l.IDLocation
						    left join DIM_MODEL as m on m.IDModel = t.IDModel
							left join DIM_MANUFACTURER as mn on mn.IDManufacturer = m.IDManufacturer
...
select Model_Name from FACT_TRANSACTIONS as t inner join DIM_LOCATION as l on t.IDLocation = l.IDLocation
							left join DIM_MODEL as m on m.IDModel = t.IDModel
where TotalPrice in (select MIN(TotalPrice) from FACT_TRANSACTIONS as t inner join DIM_LOCATION as l on t.IDLocation = l.IDLocation
							left join DIM_MODEL as m on m.IDModel = t.IDModel)
...
select Model_Name, AVG(TotalPrice) as Average_Price from FACT_TRANSACTIONS as t inner join DIM_LOCATION as l on t.IDLocation = l.IDLocation
							left join DIM_MODEL as m on m.IDModel = t.IDModel
							left join DIM_MANUFACTURER as mn on mn.IDManufacturer = m.IDManufacturer
where Manufacturer_Name in (select hg.Manufacturer_Name from (select top 5 Manufacturer_Name, SUM(Quantity) as Qty from FACT_TRANSACTIONS as t inner join DIM_LOCATION as l on t.IDLocation = l.IDLocation
							left join DIM_MODEL as m on m.IDModel = t.IDModel
							left join DIM_MANUFACTURER as mn on mn.IDManufacturer = m.IDManufacturer
							Group by Manufacturer_Name
							Order by Qty desc) as hg)
...
select Date, Customer_Name, AVG(TotalPrice) as Average_amount_spent from DIM_CUSTOMER c inner join FACT_TRANSACTIONS t on c.IDCustomer = t.IDCustomer
where YEAR(Date) = 2009
Group by Customer_Name, Date
having AVG(TotalPrice) > 500

--Q7. List if there's any model that was in the top 5 in terms of quantity, simultaneously in 2008, 2009 and 2010.

select one.Model_Name from
(select top 5 Model_Name, sum(Quantity) as Qty from FACT_TRANSACTIONS t inner join DIM_MODEL m on t.IDModel = m.IDModel
where YEAR(Date) = 2008
Group by Model_Name
order by Qty desc) one
intersect
select two.Model_Name from
(select top 5 Model_Name, sum(Quantity) as Qty from FACT_TRANSACTIONS t inner join DIM_MODEL m on t.IDModel = m.IDModel
where YEAR(Date) = 2009
Group by Model_Name
order by Qty desc) two
intersect
select three.Model_Name from
(select top 5 Model_Name, sum(Quantity) as Qty from FACT_TRANSACTIONS t inner join DIM_MODEL m on t.IDModel = m.IDModel
where YEAR(Date) = 2010
Group by Model_Name
order by Qty desc) three
...
select one.YEAR, one.Manufacturer_Name as Top_Second_Manufacturer, one.Sales from (
select Top 2 YEAR(Date) as [YEAR], Manufacturer_Name, SUM(TotalPrice) as Sales, ROW_NUMBER() over (Order By SUM(TotalPrice) desc) as rn from FACT_TRANSACTIONS t inner join DIM_MODEL m on m.IDModel = t.IDModel
																																								 left join DIM_MANUFACTURER mn on mn.IDManufacturer = m.IDManufacturer
Where YEAR(Date) = 2009
Group by Manufacturer_Name, YEAR(Date)
Order By Sales desc) one
where rn = 2
UNION
select two.YEAR, two.Manufacturer_Name as Top_Second_Manufacturer, two.Sales from (
select Top 2 YEAR(Date) as [YEAR], Manufacturer_Name, SUM(TotalPrice) as Sales, ROW_NUMBER() over (Order By SUM(TotalPrice) desc) as rn from FACT_TRANSACTIONS t inner join DIM_MODEL m on m.IDModel = t.IDModel
																																								 left join DIM_MANUFACTURER mn on mn.IDManufacturer = m.IDManufacturer
Where YEAR(Date) = 2010
Group by Manufacturer_Name, YEAR(Date)
Order By Sales desc) two
where rn = 2

--Q9. Show the manufacturers that sold cell phone in 2010 but didn't in 2009.

select Manufacturer_Name from FACT_TRANSACTIONS t inner join DIM_MODEL m on m.IDModel = t.IDModel
								  left join DIM_MANUFACTURER mn on mn.IDManufacturer = m.IDManufacturer
where YEAR(Date) = 2010
union
select Manufacturer_Name from FACT_TRANSACTIONS t inner join DIM_MODEL m on m.IDModel = t.IDModel
...
--Spend_2009,
--Spend_2010,
((Spend_2004 - Spend_2003)*100) / Spend_2003 as PD_2004,
((Spend_2005 - Spend_2004)*100) / Spend_2004 as PD_2005,
((Spend_2006 - Spend_2005)*100) / Spend_2005 as PD_2006,
((Spend_2007 - Spend_2006)*100) / Spend_2006 as PD_2007,
((Spend_2008 - Spend_2007)*100) / Spend_2007 as PD_2008,
((Spend_2009 - Spend_2008)*100) / Spend_2008 as PD_2009,
((Spend_2010 - Spend_2009)*100) / Spend_2009 as PD_2010
from (
...
AVG(case when YEAR(Date) = 2006 then TotalPrice end) as Spend_2006, 
AVG(case when YEAR(Date) = 2006 then Quantity end) as Qty_2006,
AVG(case when YEAR(Date) = 2007 then TotalPrice end) as Spend_2007, 
AVG(case when YEAR(Date) = 2007 then Quantity end) as Qty_2007,
AVG(case when YEAR(Date) = 2008 then TotalPrice end) as Spend_2008, 
AVG(case when YEAR(Date) = 2008 then Quantity end) as Qty_2008,
AVG(case when YEAR(Date) = 2009 then TotalPrice end) as Spend_2009, 
AVG(case when YEAR(Date) = 2009 then Quantity end) as Qty_2009,
AVG(case when YEAR(Date) = 2010 then TotalPrice end) as Spend_2010, 
AVG(case when YEAR(Date) = 2010 then Quantity end) as Qty_2010 
...
from (
select Customer_Name, AVG(TotalPrice) as Average_Spend, AVG(Quantity) as Avg_Qty , YEAR(Date) as [YEAR] from DIM_CUSTOMER c inner join FACT_TRANSACTIONS t on c.IDCustomer = t.IDCustomer
...
select ultimate_tier.Customer_Name, sum(case when ultimate_tier.YEAR = 2003 then ultimate_tier.Average_Spend/100 end) as spend_2003 from (
select one.Customer_Name, one.Average_Spend, one.Avg_Qty, one.YEAR from (
...
Group By Customer_Name, YEAR(Date)
Order By Average_Spend desc) one
union all
select two.Customer_Name, two.Average_Spend, two.Avg_Qty, two.YEAR from (
select Top 100 Customer_Name, AVG(TotalPrice) as Average_Spend, AVG(Quantity) as Avg_Qty , YEAR(Date) as [YEAR] from DIM_CUSTOMER c inner join FACT_TRANSACTIONS t on c.IDCustomer = t.IDCustomer
where YEAR(Date) = 2004
Group By Customer_Name, YEAR(Date)
Order By Average_Spend desc) two
union all
select three.Customer_Name, three.Average_Spend, three.Avg_Qty, three.YEAR from (
select Top 100 Customer_Name, AVG(TotalPrice) as Average_Spend, AVG(Quantity) as Avg_Qty , YEAR(Date) as [YEAR] from DIM_CUSTOMER c inner join FACT_TRANSACTIONS t on c.IDCustomer = t.IDCustomer
where YEAR(Date) = 2005
Group By Customer_Name, YEAR(Date)
Order By Average_Spend desc) three
union all
select four.Customer_Name, four.Average_Spend, four.Avg_Qty, four.YEAR from (
select Top 100 Customer_Name, AVG(TotalPrice) as Average_Spend, AVG(Quantity) as Avg_Qty , YEAR(Date) as [YEAR] from DIM_CUSTOMER c inner join FACT_TRANSACTIONS t on c.IDCustomer = t.IDCustomer
where YEAR(Date) = 2006
Group By Customer_Name, YEAR(Date)
Order By Average_Spend desc) four
union all
select five.Customer_Name, five.Average_Spend, five.Avg_Qty, five.YEAR from (
select Top 100 Customer_Name, AVG(TotalPrice) as Average_Spend, AVG(Quantity) as Avg_Qty , YEAR(Date) as [YEAR] from DIM_CUSTOMER c inner join FACT_TRANSACTIONS t on c.IDCustomer = t.IDCustomer
where YEAR(Date) = 2007
Group By Customer_Name, YEAR(Date)
Order By Average_Spend desc) five
union all
select six.Customer_Name, six.Average_Spend, six.Avg_Qty, six.YEAR from (
select Top 100 Customer_Name, AVG(TotalPrice) as Average_Spend, AVG(Quantity) as Avg_Qty , YEAR(Date) as [YEAR] from DIM_CUSTOMER c inner join FACT_TRANSACTIONS t on c.IDCustomer = t.IDCustomer
where YEAR(Date) = 2008
Group By Customer_Name, YEAR(Date)
Order By Average_Spend desc) six
union all
select seven.Customer_Name, seven.Average_Spend, seven.Avg_Qty, seven.YEAR from (
select Top 100 Customer_Name, AVG(TotalPrice) as Average_Spend, AVG(Quantity) as Avg_Qty , YEAR(Date) as [YEAR] from DIM_CUSTOMER c inner join FACT_TRANSACTIONS t on c.IDCustomer = t.IDCustomer
where YEAR(Date) = 2009
Group By Customer_Name, YEAR(Date)
Order By Average_Spend desc) seven
union all
select eight.Customer_Name, eight.Average_Spend, eight.Avg_Qty, eight.YEAR from (
select Top 100 Customer_Name, AVG(TotalPrice) as Average_Spend, AVG(Quantity) as Avg_Qty , YEAR(Date) as [YEAR] from DIM_CUSTOMER c inner join FACT_TRANSACTIONS t on c.IDCustomer = t.IDCustomer
where YEAR(Date) = 2010
Group By Customer_Name, YEAR(Date)
Order By Average_Spend desc ) eight
) ultimate_tier
group by ultimate_tier.Customer_Name
